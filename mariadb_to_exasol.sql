...
/*
    mariadb_to_exasol.sql  -  generate the statements to migrate a MariaDB database to Exasol v8.

    Source: MariaDB 10.5+ / 11.x / 12.x. This script runs on the TARGET Exasol database, reads the SOURCE
    metadata through a JDBC connection and RETURNS the statements (CREATE SCHEMA / CREATE TABLE incl. PRIMARY
    KEY / FOREIGN KEY / PARTITION BY / COMMENTs / IMPORT / a final CONSTRAINT STATE section / optional VIEW
    review section / optional DATA VALIDATION). It changes nothing itself - review the output and run it in the
    order returned.

    MariaDB is a fork of MySQL; this script shares most of the mysql_to_exasol.sql mapping but handles the
    MariaDB-specific behavior that was verified live (MariaDB 12.3, MariaDB Connector/J 3.5.9):
      * information_schema DEFAULTs are SQL-literal-formatted: a nullable column with no default reads as the
        literal string 'NULL'; string defaults are already quoted ('NEW'); CURRENT_TIMESTAMP reads as
        'current_timestamp()'. -> defaults are passed through as-is (only 'NULL' dropped, current_timestamp
        mapped); they are NOT re-quoted.
      * MariaDB-only native types: UUID -> CHAR(36) (transferred via CAST(.. AS CHAR) - the raw UUID type is not
        importable), INET4 -> VARCHAR(15), INET6 -> VARCHAR(45). JSON is an alias for LONGTEXT (no 'json' data
        type exists) and maps to VARCHAR(2000000) automatically.
      * Zero-dates (0000-00-00) are allowed by default in MariaDB and the MariaDB driver converts them to NULL on
        read -> the default TEMPORAL_OUT_OF_RANGE is 'NULL' (explicit). 'CLAMP' (-> 0001-01-01) and 'FAIL' (the
        IMPORT fails loudly; implemented by transferring the value as text so Exasol rejects the zero) are options.
      * Sequences (table_type 'SEQUENCE') are skipped; system-versioned tables (table_type 'SYSTEM VERSIONED')
        are migrated as a normal table holding their CURRENT rows (Exasol has no system-versioning; the hidden
        period columns are not migrated, and are excluded from the migrated PRIMARY KEY).

    DATA TYPE MAPPING (every MariaDB type is covered - nothing is silently dropped):
      ...
      uuid -> CHAR(36); inet4 -> VARCHAR(15); inet6 -> VARCHAR(45); binary/varbinary/*blob -> base64 text
      (BINARY_HANDLING); geometry & all spatial subtypes -> GEOMETRY (WKT).
      Small documented difference: time -> VARCHAR(17) (Exasol has no TIME type; MariaDB TIME spans -838:59:59 ..
      838:59:59 and keeps fractional seconds as text); year -> VARCHAR(4). tinyint(1) -> DECIMAL(3,0) lossless by
      default, or BOOLEAN with TINYINT1_AS_BOOLEAN=true.
      Hard limits (the IMPORT fails loudly rather than corrupting data): a value > 2,000,000 characters (unless
      TRUNCATE_LONG_STRINGS=true); DECIMAL needing > 36 digits under DECIMAL_OVERFLOW='CAP'.

    WHY CASTS ARE NEEDED ON THE SOURCE (verified live with Connector/J 3.5.9): UNSIGNED integers exceed their
    signed Java type (SMALLINT UNSIGNED 60000 overflows java.lang.Short; BIGINT UNSIGNED 18446744073709551615 and
    BIT(64) overflow java.lang.Long), so every unsigned integer / bit is transferred as text via CAST(.. AS CHAR)
    into a DECIMAL target; YEAR is returned as a DATE by the driver, so CAST(.. AS CHAR) yields '2025'; TIME via
    CAST(.. AS CHAR) keeps the full range/fraction; UUID via CAST(.. AS CHAR); binary via TO_BASE64(..) is
    byte-exact; spatial via ST_AsText(..); tinyint(1) (the driver coerces it to boolean, collapsing any non 0/1
    to 1) is read via CAST(.. AS SIGNED) so the real integer survives.
    ...
    PARTITIONING: a single-column MariaDB partition key (RANGE/LIST COLUMNS on one bare column) is mapped
    best-effort to an Exasol PARTITION BY on that column (GENERATE_PARTITION_BY); HASH/KEY/expression
    partitioning is emitted as a commented manual-review note. A MariaDB partitioned table is a single logical
    table (no separate child tables), so data is never migrated twice. MariaDB has no distribution/clustering
    concept, so no DISTRIBUTE BY is generated.

    Not migrated (out of scope): indexes, UNIQUE/CHECK constraints, triggers, routines, events, sequences. The
    history of system-versioned tables is not migrated (only the current rows). AUTO_INCREMENT is migrated as a
    plain numeric column carrying its values; generated columns (STORED/VIRTUAL) are migrated as plain columns
    carrying their stored/computed values (Exasol has no computed columns).
*/
--/
create or replace script database_migration.MARIADB_TO_EXASOL(
  CONNECTION_NAME               -- name of the JDBC connection inside Exasol -> e.g. MARIADB_JDBC
  ...
  ,GENERATE_COMMENTS            -- true/false: migrate MariaDB table/column comments as COMMENT ON
  ,GENERATE_VIEWS               -- true/false: emit source views as a commented manual-review section
  ,GENERATE_PARTITION_BY        -- true/false: add a best-effort PARTITION BY from a single-column MariaDB partition key; complex partitioning is emitted as a commented manual-review note
  ...
  ,TEMPORAL_OUT_OF_RANGE        -- 'NULL' (recommended for MariaDB; zero-dates -> NULL, matching the driver), 'CLAMP' (zero-date -> 0001-01-01) or 'FAIL' (IMPORT fails loudly on a zero-date)
...
-- MariaDB default = NULL (zero-dates are allowed/common and the driver nulls them anyway)
oormode = string.upper(tostring(TEMPORAL_OUT_OF_RANGE))
if oormode ~= 'FAIL' and oormode ~= 'CLAMP' then oormode = 'NULL' end
...
-- Always-excluded MariaDB system schemas + the requested SCHEMA/TABLE filters (inner literals double-quoted).
-- Migrated object kinds: BASE TABLE and SYSTEM VERSIONED (current rows). SEQUENCEs and VIEWs are not base data.
...
basetab = [[table_type in (''BASE TABLE'',''SYSTEM VERSIONED'')]]

-------------------------------------------------------------------------------------------------------
-- Remote (MariaDB) metadata queries. Inner literals are quote-doubled (embedded in statement '...').
-------------------------------------------------------------------------------------------------------
...
-- Exasol column type, mapped by MariaDB data_type (+ unsigned flag / display width / fractional precision).
col_t = [[case
	...
	when "data_type" = 'uuid' then 'CHAR(36) ASCII'
	when "data_type" = 'inet4' then 'VARCHAR(15) ASCII'
	when "data_type" = 'inet6' then 'VARCHAR(45) ASCII'
	...
	when "data_type" in ('tinytext','text','mediumtext','longtext','json') then 'VARCHAR(2000000) UTF8'
...
-- DEFAULT mapping. MariaDB stores SQL-literal-formatted defaults: 'NULL' (literal) = none; strings are already
-- quoted ('NEW'); CURRENT_TIMESTAMP reads as current_timestamp(). Pass literals through as-is (no re-quoting);
-- skip other expression defaults (best-effort) and generated columns.
default_e = [[case
	when "is_generated" = 1 then ''
	when "column_default" is null or "column_default" = 'NULL' then ''
	when lower("column_default") like 'current_timestamp%' then ' DEFAULT CURRENT_TIMESTAMP'
	when "column_default" REGEXP_LIKE '^[-]{0,1}[0-9]+(\.[0-9]+){0,1}$' then ' DEFAULT ' || "column_default"
	when "column_default" REGEXP_LIKE '^''.*''$' then ' DEFAULT ' || "column_default"
	else ''
end]]
coldef = [['"' || "exa_col" || '" ' || (]]..col_t..[[) || (]]..default_e..[[) || (case when "not_null" = 1 then ' NOT NULL' else '' end)]]

-- temporal source expressions (TEMPORAL_OUT_OF_RANGE; MariaDB zero-dates '0000-00-00' detected via "= 0").
-- NULL (default): explicit NULL. CLAMP: typed -> 0001-01-01. FAIL: transfer as text so Exasol rejects the zero.
if oormode == 'CLAMP' then
...
elseif oormode == 'FAIL' then
	date_src = [['case when ' || "col_bt" || ' = 0 then ''''0000-00-00'''' else cast(' || "col_bt" || ' as char) end']]
	ts_src   = [['case when ' || "col_bt" || ' = 0 then ''''0000-00-00 00:00:00'''' else cast(' || "col_bt" || ' as char) end']]
else
...
src = [[case
	...
	when "data_type" in ('year','time','uuid') then 'cast(' || "col_bt" || ' as char)'
...
elseif cstate == 'SET_AS_SOURCE' then sw = 'enable'; scomment = [[  -- matches MariaDB source (keys active)]]
...
known = [["data_type" in ('tinyint','smallint','mediumint','int','integer','bigint','decimal','numeric','float','double','real','bit','date','datetime','timestamp','time','year','uuid','inet4','inet6','char','varchar','tinytext','text','mediumtext','longtext','enum','set','json','binary','varbinary','tinyblob','blob','mediumblob','longblob','geometry','point','linestring','polygon','multipoint','multilinestring','multipolygon','geometrycollection')]]

-- optional CTEs --------------------------------------------------------------------------------------
comments_cte = ''  comments_union = ''
if gen_comments then
	comments_cte = [[
,vv_comments_raw as (select * from (import from jdbc at ]]..CONNECTION_NAME..[[ statement 'select table_schema as schema_name, table_name, 0 as sub, cast(null as char) as column_name, table_comment as comment_text from information_schema.tables where ]]..basetab..[[ and table_comment <> '''' ]]..myflt..[[ union all select table_schema, table_name, ordinal_position, column_name, column_comment from information_schema.columns where column_comment <> '''' ]]..myflt..[[ and (table_schema, table_name) in (select table_schema, table_name from information_schema.tables where ]]..basetab..[[)') c ("schema_name", "table_name", "sub", "column_name", "comment_text") )
...
,vv_views as (select '-- ' || "schema_name" || '.' || "view_name" || '  (MariaDB view - review and adapt to Exasol SQL manually):' || chr(10) || '-- ' || replace("view_def", chr(10), chr(10) || '-- ') as sql_text from vv_views_raw)]]
	views_union = "\n".. [[UNION ALL select 90, cast('-- ### VIEWS (MariaDB SQL - commented out, manual review required) ###' as varchar(2000000)) SQL_TEXT
...
,vv_part as (
	...
	union all
	select '-- "' || ]]..U('"schema_name"')..[[ || '"."' || ]]..U('"table_name"')..[[ || '" MariaDB ' || "pm" || ' partitioning (' || "pe" || ') not auto-mapped - review and add PARTITION BY manually if appropriate.' as sql_text
	from vv_partcol where "raw_col" is null)]]
	part_union = "\n".. [[UNION ALL select 37, cast('-- ### PARTITION BY (best-effort from a single-column MariaDB partition key; complex partitioning listed as a review note) ###' as varchar(2000000)) SQL_TEXT
...
-- decimals/integers only), date/timestamp MIN/MAX (full-precision text), variable-char length MIN/MAX.
-- Binary/geometry/uuid/inet get NULL/DISTINCT counts only; fixed char and DOUBLE/float are excluded.
...
,vv_chk_base as (
	...
	from vv_columns x
	cross join (select 'Exasol' as "db_system" union all select 'MariaDB' as "db_system") sysrow
...
,vv_chk_sys as (
	select "exa_schema","exa_table","schema_name","table_name","wide_name","db_system",
	       'select ' || (case when "db_system" = 'Exasol' then 'cast(''Exasol'' as varchar(10)) as "DB_SYSTEM", ' else '''MariaDB'' as db_system, ' end) || listagg("metric_expr" || (case when "db_system" = 'Exasol' then ' as "' || "metric_name" || '"' else '' end), ', ') within group (order by "ordinal_position","metric_id") || ' from ' || (case when "db_system" = 'Exasol' then '"' || "exa_schema" || '"."' || "exa_table" || '"' else '`' || "schema_name" || '`.`' || "table_name" || '`' end) as "sys_select"
	from vv_chk_named group by "exa_schema","exa_table","schema_name","table_name","wide_name","db_system"
)
,vv_chk_wide as (
	select 'CREATE OR REPLACE TABLE "' || "exa_schema" || '"."' || "wide_name" || '" AS ' || max(case when "db_system" = 'Exasol' then "sys_select" end) || ' UNION ALL select * from (IMPORT FROM JDBC AT ]]..CONNECTION_NAME..[[ STATEMENT ' || '''' || replace(max(case when "db_system" = 'MariaDB' then "sys_select" end), '''', '''''') || '''' || ') ;' as sql_text
...
,vv_chk_summary as (
	select 'CREATE OR REPLACE TABLE "DATABASE_MIGRATION"."' || "exa_schema" || '_MIG_CHK" AS select e."TABLE_NAME", e."METRIC", e."VAL" as "EXASOL_METRIC", t."VAL" as "MARIADB_METRIC", case when coalesce(e."VAL", ''~NULL~'') = coalesce(t."VAL", ''~NULL~'') then ''OK'' else ''DEVIATION'' end as "STATUS" from (' || listagg(case when "db_system" = 'Exasol' then "frag" end, ' union all ') within group (order by "exa_table","ordinal_position","metric_id") || ') e join (' || listagg(case when "db_system" = 'MariaDB' then "frag" end, ' union all ') within group (order by "exa_table","ordinal_position","metric_id") || ') t on e."TABLE_NAME" = t."TABLE_NAME" and e."METRIC" = t."METRIC" order by "STATUS" desc, e."TABLE_NAME", e."METRIC";' as sql_text
...
,vv_catchall as (
	select '-- NOTE: column "' || "schema_name" || '"."' || "table_name" || '"."' || "column_name" || '" has unmapped MariaDB type ' || "data_type" || ' -> migrated via VARCHAR(2000000) catch-all (please review).' as sql_text
...
--   * Use the latest MariaDB JDBC driver (mariadb-java-client).
--
-- JDBC driver (install once in BucketFS - the driver and its settings.cfg)
--   * mariadb-java-client 3.x or higher
--       https://mvnrepository.com/artifact/org.mariadb.jdbc/mariadb-java-client
--   * Driver setup guide:
--       https://docs.exasol.com/db/latest/loading_data/connect_sources/mariadb.htm
--   * Note: the MariaDB driver converts zero-dates (0000-00-00) to NULL by default, which matches the
--     recommended TEMPORAL_OUT_OF_RANGE='NULL'. Use 'FAIL' to reject them or 'CLAMP' to map them to 0001-01-01.
--
-- Create a connection to the MariaDB database (adjust host, database name and credentials),
-- then run the accompanying test query.

CREATE OR REPLACE CONNECTION MARIADB_JDBC
    TO 'jdbc:mariadb://mariadb_host_or_ip:3306/my_database'
    USER 'username' IDENTIFIED BY 'password';
SELECT * FROM (IMPORT FROM JDBC AT MARIADB_JDBC STATEMENT 'SELECT ''Connection works''');
