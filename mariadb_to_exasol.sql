...
CONNECTION_NAME 				-- name of the database connection inside exasol
,IDENTIFIER_CASE_INSENSITIVE 	-- true if identifiers should be stored case-insensitiv (will be stored upper_case)
,SCHEMA_FILTER 					-- filter for the schemas to generate and load (except 'information_schema' and 'mysql' and 'performance_schema') -> '%' to load all
...
with vv_mysql_columns as (
    ...
    ( import from jdbc at ]]..CONNECTION_NAME..[[ statement
        'select table_catalog, table_schema, table_name, column_name, ordinal_position, case when column_default=''NULL'' then '''' when column_default is null then '''' when column_default like ''current_timestamp%'' then ''CURRENT_TIMESTAMP'' when column_default is not null and data_type=''timestamp'' then '''' else column_default end as COLUMN_DEFAULT_X, case when is_nullable=''NO'' then ''NOT NULL'' else '''' end as NOT_NULL_CONSTRAINT, data_type, column_type, character_maximum_length, numeric_precision, numeric_scale
...
,vv_create_tables as (
    ...
    case
    -- ### numeric types ###
    when upper(data_type) = 'INT' then '"' || "exa_column_name" || '" ' || 'DECIMAL(11,0) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'INTEGER' then '"' || "exa_column_name" || '" ' || 'DECIMAL(11,0) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'TINYINT' then '"' || "exa_column_name" || '" ' || 'DECIMAL(4,0) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'SMALLINT' then '"' || "exa_column_name" || '" ' || 'DECIMAL(5,0) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'MEDIUMINT' then '"' || "exa_column_name" || '" ' || 'DECIMAL(9,0) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'BIGINT' then '"' || "exa_column_name" || '" ' || 'DECIMAL (20,0) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'FLOAT' then '"' || "exa_column_name" || '" ' || 'FLOAT ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'DOUBLE' then '"' || "exa_column_name" || '" ' || 'DOUBLE ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    -- in mysql scale <= 30 and scale <= precision
    when upper(data_type) = 'DECIMAL' then case when numeric_precision is null then '"' || "exa_column_name" || '" ' || 'DOUBLE' else '"' || "exa_column_name" || '" ' || 'decimal(' || case when numeric_precision > 36 then 36 else numeric_precision end || ',' || case when (numeric_scale > numeric_precision) then numeric_precision else  case when numeric_scale < 0 then 0 else numeric_scale end end || ') ' end || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    ...
    */
    when upper(data_type) = 'BIT' then '"' || "exa_column_name" || '" ' || 'DECIMAL('||numeric_precision||',0) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT

    -- ### date and time types ###
    when upper(data_type) = 'DATE' then '"' || "exa_column_name" || '" ' || 'DATE ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'DATETIME' then '"' || "exa_column_name" || '" ' || 'TIMESTAMP ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'TIMESTAMP' then '"' || "exa_column_name" || '" ' || 'TIMESTAMP ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'TIME' then '"' || "exa_column_name" || '" ' || 'varchar(8) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'YEAR' then '"' || "exa_column_name" || '" ' || 'varchar(4) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT

    -- ### string types ###
    when upper(data_type) = 'CHAR' then '"' || "exa_column_name" || '" ' || upper(column_type) || ' ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'VARCHAR' then '"' || "exa_column_name" || '" ' || upper(column_type) || ' ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'BINARY' then '"' || "exa_column_name" || '" ' || 'char('||character_maximum_length||') ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'VARBINARY' then '"' || "exa_column_name" || '" ' || 'varchar('||character_maximum_length||') ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'TINYTEXT' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'TEXT' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'MEDIUMTEXT' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'LONGTEXT' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'TINYBLOB' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'BLOB' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'MEDIUMBLOB' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'LONGBLOB' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'ENUM' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
    when upper(data_type) = 'SET' then '"' || "exa_column_name" || '" ' || 'varchar(2000000) ' || case when COLUMN_DEFAULT_X is not NULL then 'DEFAULT ' || COLUMN_DEFAULT_X || ' ' end || NOT_NULL_CONSTRAINT
...
'JDBC_MARIADB' --name of your database connection
...
,'%TEST%' -- schema filter --> '%' to load all schemas except 'information_schema' and 'mysql' and 'performance_schema' / '%publ%' to load all schemas like '%pub%'
,'%TEST%' -- table filter --> '%' to load all tables / '%publ%' to load all tables like '%pub%'
);
