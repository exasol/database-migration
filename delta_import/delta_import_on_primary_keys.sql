...
--query all needed information for schema name and table name within one resultset -> go through resultset afterwards
-- outputs: table_schema (target) table_name all_columns key_columns no_key_columns
...
for target_schema, target_table, all_columns, key_columns, no_key_columns in res_key_col_infos do 
	...
	wrapper:set_param('selectStarStmt', selectStarStmt)
 	importFromSrc = [[ from ::conn_type at ::conn_name statement :selectStarStmt]]
	...
	if key_columns == NULL then
		...
		suc, currRowCount = wrapper:query([[import into ::curr_schema.::curr_table]].. importFromSrc)
		...
		local success, maxDeltaVal = pquery([[select ]].. maxQuery ..[[ from ::curr_schema.::curr_table]], {col_name=delta_detection_column, curr_schema=target_schema, curr_table=target_table});

		if not success then	--delta-detection-column doesn't exist in the table-> do a truncate and then a load
			wrapper:query([[truncate table ::curr_schema.::curr_table]]);
			suc, currRowCount = wrapper:query([[import into ::curr_schema.::curr_table]]..importFromSrc)
			local tmpRowCount = currRowCount.etl_rows_written
			...
			if maxDeltaVal[1][1]== NULL then	-- no maximum in delta detection column found  -> do a full load into the staging table
				suc, currRowCount = wrapper:query([[import into ::staging_schema.::curr_table]]..importFromSrc)
				local tmpRowCount = currRowCount.etl_rows_written
